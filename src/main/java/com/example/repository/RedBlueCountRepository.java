...
public interface RedBlueCountRepository extends Repository<RedBlueCount, Long> {
    @Query(value="select * from double_ball_blue_red_relations a where a.blue = :blue AND a.create_time BETWEEN :startTime AND :endTime ORDER BY a.red ASC ",nativeQuery=true)
